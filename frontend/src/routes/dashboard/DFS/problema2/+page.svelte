<script lang="ts">
  ...
  const problema2 = {
    titlu: "Problema 2: Căutare DFS (Timisoara - Bucharest)",
    cerinta: "Folosind harta din stânga, construiți arborele de căutare rezultat prin aplicarea algoritmului Depth-First Search (DFS). Extindeți vecinii în ordine alfabetică. Starea de start este Timisoara, iar starea obiectiv este Bucharest.",
    ...
    // Soluția corectă DFS
    solutiePerfecta: [
      { nod: 'Timisoara', parinte: null },
      { nod: 'Arad', parinte: 'Timisoara' },
      { nod: 'Lugoj', parinte: 'Timisoara' },
  ...
  onMount(() => {
    noduriArbore = [{ id: idContor++, eticheta: 'Timisoara', parinteId: null, taiat: false }];
...
<style>
  /* Căsuța de style este absolut aceeași cu cea de la problema 1 */
