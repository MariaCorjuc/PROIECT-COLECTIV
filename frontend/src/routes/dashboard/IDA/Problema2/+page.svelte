<script>
    ...
    import { calculeazaPunctajIDA } from './dateArbore.js';

    let pornit = $state(false);
    let statusValidare = $state(''); 

    let numarIteratieCurenta = $state(1); 
    let limitaCurenta = $state('0');
    
    let dejaRezolvata = $state(false);

     onMount(() => {
        if (localStorage.getItem('problema_2_ida_rezolvata') === 'true') {
            dejaRezolvata = true;
        }
    ...
    let noduriDesenate = $state([
        { idUnique: 'A_0', label: 'A', x: 400, y: 60 }
    ]);
    ...
    function marcheazaDepasesteLimita() {
    if (!nodSursaSelectat || statusValidare !== '' || dejaRezolvata) return;

    if (!idNoduriDepasite.includes(nodSursaSelectat)) {
        idNoduriDepasite = [...idNoduriDepasite, nodSursaSelectat];
    } else {
        idNoduriDepasite = idNoduriDepasite.filter(id => id !== nodSursaSelectat);
    }
    ...
    let noduriStudent = noduriDesenate.map(n => n.label);

    let rezultat = calculeazaPunctajIDA(numarIteratieCurenta, Number(limitaCurenta), noduriStudent);

    if (rezultat.corect) {
        mesajFeedback = rezultat.mesaj;

        if (rezultat.status === 'finalizat') {
            punctajCurent = 100;
            statusValidare = 'corect';
            localStorage.setItem('problema_2_ida_rezolvata', 'true');
            dejaRezolvata = true;
        ...
        } else if (rezultat.status === 'iteratie_ok') {
            statusValidare = 'pas_corect'; 
        }
    ...
    function restartProblema() {
        if (dejaRezolvata) return; 
        idCounter = 0;
        noduriDesenate = [{ idUnique: 'A_0', label: 'A', x: 400, y: 60 }];
        muchiiDesenate = [];
        drumStudent = [];
        idNoduriVizitate = []; 
        punctajCurent = 0;
    ...
    function treciLaUrmatoareaIteratie(){
        numarIteratieCurenta+=1;
        noduriDesenate = [{idUnique: 'A_0', label: 'A', x:400, y:60}];
        muchiiDesenate=[];
        drumStudent=[];
        idNoduriVizitate=[];
...
<div class="ecran-start" style:display={pornit ? 'none' : 'flex'}>
    <h1>Problema 2: Iterative Deepening A* Search</h1>
    {#if dejaRezolvata}
...
<div bind:this={zonaProblema} class="container-problema-noua" style:display={pornit ? 'block' : 'none'}>
    ...
    <div class="bara-enunt-global">
        <div class="continut-text">
            <button class="btn-inapoi" onclick={inapoiLaTeorie}>
                ← Înapoi la pagina IDA
            </button>
            <h1>Problema 2:</h1>
            <p>Fie graful ponderat alăturat; rezolvați problema determinării drumului de cost minim de la nodul de start S la nodul final G aplicând algoritmul IDA*. Atunci când introduceți copiii unui nod, aceștia se vor genera în ordine alfabetică.</p>
        </div>
        
        <div class="zona-imagine-graf">
            <img src="/poze/problema_2_ida.jpg" alt="Graful problemei UCS" class="poza-graf-noua">
        </div>
    </div>
    ...
    <div class="spatiu-rezolvare">
            ...
            >
                <g transform="translate({panX}, {panY}) scale({scale})">
                    ...
                    {#each noduriDesenate as nod}
                        ...
                        {#if idNoduriDepasite.includes(nod.idUnique)}
                            <text 
                                x={nod.x + 28} 
                                y={nod.y + 5} 
                                text-anchor="start" 
        ...
        <div class="cadran-noduri">
            ...
            <div class="zona-limita-input" style="margin-bottom: 15px;">
                <label for="limita" style="font-weight: bold; font-size: 14px; color: #2c3e50;">Setează Limita Curentă (fLimit):</label>
                <input type="number" id="limita" bind:value={limitaCurenta} min="0" style="width: 100%; padding: 8px; margin-top: 5px; border: 1px solid #cbd5e1; border-radius: 6px;" />
            </div>
            ...
            {#if !asteaptaValoare}
                ...
                <div class="grid-noduri" style="margin-bottom: 15px;">
            ...
            {:else}
                <div class="zona-introducere-cost" style="margin-top: 15px; margin-bottom: 15px;">
                    ...
                    <input type="number" bind:value={valoareIntrodusa} placeholder="f(n)=g(n)+h(n)" min="0" />
            ...
            {#if nodSursaSelectat && !asteaptaValoare && statusValidare === '' && !dejaRezolvata}
                <div class="zona-depaseste-limita">
                    <button class="btn-depaseste-limita" onclick={marcheazaDepasesteLimita}>
                        Nodul depășește limita
                    </button>
                </div>
            {/if}
<div class="zona-control-jos">
    ...
    {:else if statusValidare === 'pas_corect'}
        <div class="mesaj-succes" style="background: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd;">
            {mesajFeedback}
            <button onclick={treciLaUrmatoareaIteratie} class="btn-iteratie-urmatoare">
                →
            </button>
        </div>
    {/if}

    {#if statusValidare === '' && !dejaRezolvata && !asteaptaValoare}
...
<style>
    .zona-depaseste-limita {
        margin-top: 10px;
        margin-bottom: 20px;
        width: 100%;
    }
    .btn-depaseste-limita {
        width: 100%;
