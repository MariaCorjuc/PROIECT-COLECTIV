<script lang="ts">
  ...
  // Set diferit de frunze pentru Problema 2
  const frunze = [8, 2, 5, 9, 1, 3, 7, 4, 6, 10, 11, 2, 1, 8, 4, 3, 7, 5, 6, 9, 1, 2, 8, 3, 4, 5, 12];
  ...
  function calculeazaValoareCorecta(id: number): number {
    ...
    if (id === 0 || (id >= 4 && id <= 12)) return Math.max(c1, c2, c3);
    return Math.min(c1, c2, c3);
  }

  function getX(id: number): number {
    if (id >= 13) return ((id - 13) * 55) + 30;
    return (getX(id * 3 + 1) + getX(id * 3 + 3)) / 2;
  }
...
<div class="ecran-complet-student" bind:this={containerLaborator}>
  <div class="antet-cerinta">
    <div class="grup-text-cerinta">
      <h2>Problema 2: Completează Arborele Minimax</h2>
...
<style>
  /* Căsuța de <style> rămâne absolut identică cu cea de la Problema 1 pentru consistența designului */
