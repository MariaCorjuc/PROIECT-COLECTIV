<script>
    ...
    let dejaRezolvata = $state(false);
     ...
     onMount(() => {
        if (localStorage.getItem('problema_2_ucs_rezolvata') === 'true') {
            dejaRezolvata = true;
        }
    ...
    const toateNodurile = ['A', 'B', 'D', 'K', 'F', 'E', 'H', 'G','I','J'];
    ...
    const verificaRezolvare = async () => {
        ...
        if (scorCalculat === 100) {
            ...
            localStorage.setItem('problema_2_ucs_rezolvata', 'true');
            dejaRezolvata = true;
        } else {
            punctajCurent = 0;
...
<div class="ecran-start" style:display={pornit ? 'none' : 'flex'}>
    <h1>Problema 2: Uniform Cost Search</h1>
    {#if dejaRezolvata}
...
<div bind:this={zonaProblema} class="container-problema-noua" style:display={pornit ? 'block' : 'none'}>
    ...
    <div class="bara-enunt-global">
        <div class="continut-text">
            ...
            <h1>Problema 2:</h1>
            <p>Fie graful ponderat alăturat; rezolvați problema determinării drumului de cost minim de la nodul de start A la nodul final G aplicând algoritmul de căutare cu cost uniform (UCS). Atunci când introduceți copiii unui nod, aceștia se vor genera în ordine alfabetică.</p>
        </div>
        
        <div class="zona-imagine-graf">
            <img src="/poze/problema_2_ucs.png" alt="Graful problemei UCS" class="poza-graf-noua">
        </div>
    </div>
