...
var getDirection$1 = ({ source, sourcePosition = Position.Bottom, target }) => {
...
var reconnectEdge = (oldEdge, newConnection, edges, options = { shouldReplaceId: true }) => {
	...
	if (!edges.find((e) => e.id === oldEdgeId)) {
...
var getTransformForBounds = (bounds, width, height, minZoom, maxZoom, padding = .1) => {
...
var selector$3$1 = (s) => `translate(${s.transform[0]}px,${s.transform[1]}px) scale(${s.transform[2]})`;
function Viewport({ children }) {
	const transform = useStore(selector$3$1);
	return import_react.createElement("div", {
	...
	return (0, import_react.useMemo)(() => {
		{
			...
			if (shallow$1(typesKeysRef.current, typeKeys)) store.getState().onError?.("002", errorMessages["error002"]());
...
var selector$1$2 = (s) => s.domNode?.querySelector(".react-flow__edgelabel-renderer");
...
function useUpdateNodeInternals() {
	...
	return (0, import_react.useCallback)((id) => {
		...
		const updates = (Array.isArray(id) ? id : [id]).reduce((res, updateId) => {
...
function createUseItemsState(applyChanges) {
	return (initialItems) => {
		...
		return [
			items,
			setItems,
			(0, import_react.useCallback)((changes) => setItems((items) => applyChanges(changes, items)), [])
		];
	};
}
...
var defaultOptions = { includeHiddenNodes: false };
function useNodesInitialized(options = defaultOptions) {
	return useStore(selector$10(options));
}
//#endregion
...
var selector$1$1 = (s) => s.nodeOrigin;
...
function MiniMap({ style, className, nodeStrokeColor = "transparent", nodeColor = "#e2e2e2", nodeClassName = "", nodeBorderRadius = 5, nodeStrokeWidth = 2, nodeComponent, maskColor = "rgb(240, 240, 240, 0.6)", maskStrokeColor = "none", maskStrokeWidth = 1, position = "bottom-right", onClick, onNodeClick, pannable = false, zoomable = false, ariaLabel = "React Flow mini map", inversePan = false, zoomStep = 10, offsetScale = 5 }) {
	...
	const { boundingRect, viewBB, rfId } = useStore(selector$3, shallow$1);
	...
	(0, import_react.useEffect)(() => {
		if (svg.current) {
			const selection = select_default$1(svg.current);
...
//#endregion
//#region node_modules/@reactflow/node-toolbar/dist/esm/index.mjs
...
function NodeToolbar({ nodeId, children, className, style, isVisible, position = Position.Top, offset = 10, align = "center", ...rest }) {
	...
	const nodes = useStore((0, import_react.useCallback)((state) => {
		return (Array.isArray(nodeId) ? nodeId : [nodeId || contextNodeId || ""]).reduce((acc, id) => {
	...
	if (!(typeof isVisible === "boolean" ? isVisible : nodes.length === 1 && nodes[0].selected && selectedNodesCount === 1) || !nodes.length) return null;
	...
	return import_react.createElement(NodeToolbarPortal, null, import_react.createElement("div", {
		style: wrapperStyle,
...
function ResizeControl({ nodeId, position, variant = ResizeControlVariant.Handle, className, style = {}, children, color, minWidth = 10, minHeight = 10, maxWidth = Number.MAX_VALUE, maxHeight = Number.MAX_VALUE, keepAspectRatio = false, shouldResize, onResizeStart, onResize, onResizeEnd }) {
	...
	(0, import_react.useEffect)(() => {
		...
		const selection = select_default$1(resizeControlRef.current);
...
function NodeResizer({ nodeId, isVisible = true, handleClassName, handleStyle, lineClassName, lineStyle, color, minWidth = 10, minHeight = 10, maxWidth = Number.MAX_VALUE, maxHeight = Number.MAX_VALUE, keepAspectRatio = false, shouldResize, onResizeStart, onResize, onResizeEnd }) {
	if (!isVisible) return null;
	return import_react.createElement(import_react.Fragment, null, lineControls.map((c) => import_react.createElement(ResizeControl$1, {
		key: c,
		...
		onResize,
		onResizeEnd
	})), handleControls.map((c) => import_react.createElement(ResizeControl$1, {
		key: c,
...
//#endregion
export { Background$1 as Background, BackgroundVariant, BaseEdge, BezierEdge, ConnectionLineType, ConnectionMode, ControlButton, Controls$1 as Controls, EdgeLabelRenderer, EdgeText$1 as EdgeText, Handle$1 as Handle, MarkerType, MiniMap$1 as MiniMap, ResizeControl$1 as NodeResizeControl, NodeResizer, NodeToolbar, PanOnScrollMode, Panel, Position, ReactFlow, ReactFlow as default, ReactFlowProvider, ResizeControlVariant, SelectionMode, SimpleBezierEdge, SmoothStepEdge, StepEdge, StraightEdge, addEdge, applyEdgeChanges, applyNodeChanges, boxToRect, clamp, getBezierPath, getBoundsOfRects, getConnectedEdges, getIncomers, getMarkerEnd, getNodePositionWithOrigin, getNodesBounds, getOutgoers, getRectOfNodes, getSimpleBezierPath, getSmoothStepPath, getStraightPath, getTransformForBounds, getViewportForBounds, handleParentExpand, internalsSymbol, isEdge, isNode, reconnectEdge, rectToBox, updateEdge, useEdges, useEdgesState, useGetPointerPosition, useKeyPress, useNodeId, useNodes, useNodesInitialized, useNodesState, useOnSelectionChange, useOnViewportChange, useReactFlow, useStore, useStoreApi, useUpdateNodeInternals, useViewport };
